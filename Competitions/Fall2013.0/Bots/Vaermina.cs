...
namespace StarshipRepair.Bots
{
    public class Vaermina : RunnedBot
    {
        public override Command MakeTurn()
        {
            ...
            {
                int posX =
                    (int)Math.Round((Competitions.World.Robots[1 - ControlledRobot].GetAbsoluteLocation().X + 150) / 50);
                if (posX > 5) posX = 5;
                int posY =
                    (int) Math.Round((100-Competitions.World.Robots[1 - ControlledRobot].GetAbsoluteLocation().Y)/50);
                if (posY > 3) posY = 3;
                var pth =
